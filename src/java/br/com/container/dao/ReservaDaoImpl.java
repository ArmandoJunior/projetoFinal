...
public class ReservaDaoImpl extends BaseDaoImpl<Reserva, Long> implements ReservaDao {
    ...
    public static void main(String[] args) {
        ...
        List<ChartReservaMes> todas = daoImpl.totalMesSala(session);
        System.out.println(todas);
        session.close();
    }

//    @Override
//    public List<GraficoReservaPorMesSala> totalMesSala(Session session) throws HibernateException {
//        Object[] item;
//        GraficoReservaPorMesSala grt;
//        List<GraficoReservaPorMesSala> todosGrts = new ArrayList<>();
//        Query consulta = session.createQuery("select month(inicio) as mes, count(id) as qtd from Reserva group by month(inicio)");
//        
//        
...
//            grt = new GraficoReservaPorMesSala((int)item[0], (int)(long)item[1]);
//            todosGrts.add(grt);
//        }
//        return todosGrts;
//        
//    }
    ...
    @Override
    public List<ChartReservaMes> totalMesSala(Session session) throws HibernateException {
        
        Object[] item;
        ChartReservaMes sala;
        List<ChartReservaMes> salas = new ArrayList<>();
        
        Query consulta = session.createQuery("SELECT MONTH(inicio) AS mes, COUNT(id) AS totalDeSalas "
                + "FROM Reserva GROUP BY MONTH(inicio)");
        ...
        for (Object resultado : resultados) {
            ...
            item = (Object[]) resultado;
            sala = new ChartReservaMes((int) item[0], (long) item[1]);
            salas.add(sala);
        }
        return salas;
